...
> Phase 1 MVP は Tier 1 スタック (Termius Free + Tailscale + mosh + tmux) のセットアップ支援・検証・診断。
> 継続コスト $0 / 月。
...
| **Verify** | install 後 | `scripts/verify-tier1.sh` 実行 → 6 項目チェック (Tailscale up / mosh loopback / tmux / claude CLI / SSH key / caffeinate) | `scripts/verify-tier1.sh` |
| **Troubleshoot** | エラー発生時 | `scripts/doctor.sh` 実行 → 6 層診断 (Tailscale / SSH / mosh / tmux / Claude CLI / Termius config) → 症状別修復提案 | `scripts/doctor.sh`, `references/troubleshooting.md` |
...
6. caffeinate LaunchAgent がロード済か (Phase 2 拡張、Phase 1 は WARN のみ)

全 PASS で「Phase 1 セットアップ完了候補」、1 つでも FAIL なら Troubleshoot へ自動ルーティング。
...
| L6 Termius config | (iOS 側は手動確認) | `references/troubleshooting.md` §L6 |
...
1. 別 Mac / 別 iPhone の同構成で `install.sh` → `verify-tier1.sh` が 15 分以内で PASS する
2. `doctor.sh` が 6 層すべてチェックできる
...
Phase 1 スコープ外 (Phase 2 以降で追加):

- `references/claude-remote.md` (Phase 2)
- `references/mac-awake.md` (Phase 2)
